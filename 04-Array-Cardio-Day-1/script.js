...
const lifeYear=inventors.reduce((total,inventor)=>{
  return total + (inventor.passed - inventor.year);
}, 0);
console.log(lifeYear);

// 5. Sort the inventors by years lived
const highestLife=inventors.sort((a,b)=>{
  const oldFellow=a.passed - a.year;
  const newFellow=b.passed - b.year;
  return oldFellow>newFellow?-1:1;
});
console.table(highestLife);
...
// ategory=document.querySelector('.mw-category');
...
const alphaOrder=people.sort((last,first)=>{
...
const transport=data.reduce((vehical,item)=>{
  if(!vehical[item]){
    vehical[item]=0;
  }
